...
export const createPhaserTilemap = ({
  ...
  for (const cell of placed.values())
    for (const tile of tilesFor(cell, layers.filter((layer) => layer.visible))) {
  ...
  const tilesets = used.map((asset) => {
    const columns = Math.max(1, Math.floor((asset.image?.width || tileSize) / tileSize));
    const rows = Math.max(1, Math.floor((asset.image?.height || tileSize) / tileSize));
    const tileset = {
      firstgid: nextGid,
      columns,
  ...
  for (const [cellKey, cell] of placed) {
    ...
    for (const layer of layers.filter((item) => item.visible)) {
      ...
      attributeObjects.push({
        ...
        width: tileSize,
        height: tileSize,
        properties: Object.entries(tile.attributes).filter(([, value]) => value !== undefined).map(([name, value]) => ({ name, type: typeof value, value })),
      });
    }
  }
  return {
    ...
    tileheight: tileSize,
    layers: [
      ...layers.filter((layer) => layer.visible).map((layer, id) => ({
        id: id + 1,
        name: layer.name,
        type: "tilelayer",
        x: 0,
        ...
        properties: [
          ...
          ...Object.entries(layer.properties || {}).map(([name, value]) => ({ name, type: typeof value, value })),
        ],
