...
An autotile layer uses a selected 3×3 source block and configurable rules. Each surrounding cell in a rule can be required, empty, or ignored; the matching rule selects its configured variant tile. The default source tile is used when no rule matches. Tile attributes (`collision`, `hazard`, `ladder`, `spawn`, `damage`, `animated`, and custom values) apply to new paint strokes.
